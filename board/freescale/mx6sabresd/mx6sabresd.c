...
void set_panel_env(void)
{
	...
	if(LVDS_PORT==0)
	{
		env_set("lvds_num","0");//lvds0 route to di 0
		env_set("disp_num","0");//lvds0 route to di 0
	}
	else
	{
		env_set("lvds_num","1");//lvds1 route to di 1
		env_set("disp_num","1");//lvds1 route to di 1
	}
	...
	if(get_eeprom_efficient_config() == 0) { 
		// use EDID config
		env_set("panel",(char *)get_edid_eeprom_panel_name());
		return;
	}
#endif
...
int board_late_init(void)
{
	...
	switch(eeprom_i2c_get_EDID())
	{
		//env_set("bootargs","console=ttymxc0,115200 init=/init video=mxcfb0:dev=ldb,800x480M@70,if=RGB666,bpp=32 video=mxcfb1:off video=mxcfb2:off fbmem=40M fb0base=0x27b00000 vmalloc=400M androidboot.console=ttymxc0 androidboot.hardware=freescale mem=1024M\0");
		case RESOLUTION_640X480:
